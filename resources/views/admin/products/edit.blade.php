...
@section('page-title', 'Modifier : ' . $article->titre)
...
@section('content')
    <div class="max-w-2xl">
        ...
        <form action="{{ route('admin.products.update', $article) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 transition resize-none">{{ old('description', $article->description) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    <label for="prix" class="block text-sm font-semibold text-gray-700">Prix (€)</label>
                    <input type="number" id="prix" name="prix" value="{{ old('prix', $article->prix) }}" step="0.01" min="0"
                           class="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 transition">
                </div>
                <div class="space-y-2">
                    <label for="note" class="block text-sm font-semibold text-gray-700">Note (1-5)</label>
                    <input type="number" id="note" name="note" value="{{ old('note', $article->note) }}" min="1" max="5"
                           class="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 transition">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="categorie" name="categorie" value="{{ old('categorie', $article->categorie) }}" list="categories-list"
                       class="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 transition">
            ...
            <div class="space-y-2">
                <label for="image" class="block text-sm font-semibold text-gray-700">Image (laisser vide pour garder l'actuelle)</label>
                @if ($article->image)
                    <div class="mb-3">
                        <img src="{{ $article->image }}" alt="{{ $article->titre }}" class="w-32 h-32 object-cover rounded-xl ring-1 ring-gray-200">
                    </div>
                @endif
